...
static PotentialFunction sumToAddNormIdf(PotentialFunction pot)
{
   ...
   for (i=1; i< tama;i++){
      ...
      normIdf.addArguments(((Double) arg.elementAt(0)).doubleValue());
			normIdf.addArguments(((Double) arg.elementAt(1)).doubleValue());
			normIdf.addArguments(((Double) arg.elementAt(i+1)).doubleValue());
			
			

			addIdf.addArguments(normIdf);
...
public Potential restrictFunctionToVariable(
	...
	for (i=1;i<tama;i++){
	    if ( conf.getValue(i) == 1)
		{
		  aux = (arg[i+1]/Math.log(arg[0]));
	          auxPDT = aux;  //+ (1.0 -aux)*(1.0/Math.log(arg[1]));
		  // System.out.println("\n I=" +i+"arg[i+1]" + arg[i+1] + "arg[0]"+arg[0]+"aux "+aux+"auxPDT " +auxPDT);
		}
	    else auxPDT = 1.0/arg[0];
	    //System.out.println("\ni="+i+ " con.val = " + conf.getValue(i));

	    if (conf.getValue(0) == 0) auxPDT  = 1-auxPDT;
	    if (conf.getValue(0) == 1) // Calculating document relevance probability
           prDocTer = prDocTer + (1-prDocTer)*auxPDT;
        else prDocTer = prDocTer + (1-prDocTer)*(1-auxPDT);	
	
	    //System.out.println("aux PDT " + auxPDT + "prDT " + prDocTer);

	}
       if (conf.getValue(0) == 1)
	    return prDocTer;
	else return 1.0-prDocTer;
