...
/**
 * Dettach the adjacent links for a node of the graph
 * 
 * @param n the node to remove.
 */

public void removeLinksNode(Node node) throws InvalidEditException {
...
public void removeNOrientedLinksNode(Node node) throws InvalidEditException {
  ...
  if (node != null){
	...
	sibling = l.getHead();
      sibling.getSiblings().removeLink(sibling.getSiblings().indexOf(l));
//      System.out.println("Removing non-oriented link " + l.getHead().getName() + " with " + l.getTail().getName()); 
      linksl.removeLink(l);
      linkList.removeLink(l);
 ...
 */
  ...
  //   if ((post==-1)||(posh==-1)){ System.out.print("Nodos no encontrados ");}
      if ((post!=-1)&&(posh!=-1)){
...
/**
 * Calculates the neighbour nodes by non-directed links of the given node
 * (includes parents, children and siblings).
 * @param node node whose neighbours are returned.
 * @return a <code>NodeList</code> with the neighbours of <code>node</code>.
 */

public NodeList nonorientedneighbours(Node node) {
...
public boolean isThereMixedPath(Node a, Node b, Node c) {
  ...
  for (int i=0 ; i<nodeList.size() ; i++) {
    n = (Node) nodeList.elementAt(i);
    if (n.equals(c))  {  n.setVisited(true);}
    else  {  n.setVisited(false);}
  }
...
public boolean[][] computeDepMatrix(NodeList list){
   int N,i,j;
   Node a,b;
   boolean[][] matrix;
   ...
   for(i=0;i<N;i++){
       a = (Node) list.elementAt(i);
//       System.out.println("Computing nodes linked to " + a.getName());
       for ( j=0 ; j<nodeList.size() ; j++) {
      b = (Node) nodeList.elementAt(j);
       b.setVisited(false);
       }
       a.setVisited(true);
       RecisDepNodes(a, false);
       for ( j=0 ; j<N ; j++) {
            b = (Node) list.elementAt(j);
            if (b.getVisited()){
              matrix[i][j] = true;
 //                 System.out.println("Linked to " + b.getName());
            }
...
public boolean[][] computeDepMatrix2(NodeList list){
   int N,i,j;
   Node a,b;
   boolean[][] matrix;
...
private void RecisDepNodes(Node a, boolean direct) {

 {


   Enumeration e = parents(a).elements();
   Node ady;
   ...
   while (e.hasMoreElements()) {
       ady = (Node) e.nextElement();
        ...
        if (!ady.getVisited()) {
           ...
           RecisDepNodes(ady,false);
       

 } //else
   
    } 
   e = children(a).elements();
   while (e.hasMoreElements()) {
     ady = (Node) e.nextElement();

     if (!ady.getVisited()) {
       ady.setVisited(true);
       RecisDepNodes(ady,true);
     }
   }
...
private void RecisDepNodes2(Node a) {
...
public NodeList getIndep(Node n){
    NodeList l,ch,pa,aux;
    Node nx;
    int i,cs;
...
public void undirectedAccessibles(Node node, NodeList nodes) {
 ...
 if (nodes.getId(node) == -1)  {nodes.insertNode(node);}
