...
/*
* A new constructor based on a simple description of nodes
* Each node is defined by one line in a file (a string) containing the data separated by commas
* The first field is the node name
* The second field is the type 0: continuous, 1: discrete, 3: Mixed (continuous discretized node)
* The remaining fields contain: the lower and upper limits for continuous nodes
*                    The number of cases and the names of the cases for discrete nodes
*                    The number of intervals and the intervals limits for mixed nodes
*/

public NodeList(BufferedReader file) throws IOException {
    String line=null;
    String[] parts;
    int typeofnode;
    Node x,y;
   ...
   while ((line = file.readLine()) != null) {
       ...
       typeofnode = Integer.parseInt(parts[1]);
       switch (typeofnode) {
           case   1: {
               stats = new Vector();
               n = Integer.parseInt(parts[2]);
               for (i=0; i<n; i++)  {
                stats.addElement(parts[i+3]);
           }             
               
              x = new FiniteStates(parts[0].trim(),stats); 
      //         System.out.println("Node "+ parts[0] + " created ");
              break;
          
           }  
     
       case   0: {
               vmin = Double.parseDouble(parts[2]);
               vmax = Double.parseDouble(parts[3]);
                       
               
                x = new Continuous(parts[0].trim(),vmin,vmax); 
             //   System.out.println(parts[0] + "vmin " + vmin + " vmax " + vmax );
                break;
       ...
       case   3: {
               ...
               n = Integer.parseInt(parts[2]);
               limits = new double[n+1];
               for (i=0; i<=n; i++)  {
                  limits[i] = Double.parseDouble(parts[i+3]);  
           }             
              
              x = new ContinuousDiscrete(parts[0].trim(),n,limits); 
        //      System.out.println("Node "+ parts[0] + " created ");
              break;
  ...
  for (i=0 ; i < size() ; i++) {
    ...
    if (type == node.FINITE_STATES)
      size *= ((FiniteStates) node).getNumStates();
         ...
         if (type == node.MIXED)
               size *= ((ContinuousDiscrete) node).getNumberofIntevals();
         else {
            if (kind == node.CHANCE) {
	/*
...
/*
* It carries a cleaning of the graph structure associated to all the nodes of the list
 */
