...
/**
 * This method adds a row of observations to the database of cases
 * vector of cases
 * 
 */

public void addCase(int[] obser) {
  
  setNumberOfCases(getNumberOfCases()+1);
  cases.add(obser);

}


public void setCase(int[] obser,int j) {
  
  
  cases.setElementAt(obser, j);
...
/* 
* This procedure computes de vector of frequencies with a temporal information, parents are considered to be
* the variables in previous time.
*/


public double[][] getFrequencies (Node var,NodeList parents) {
    ...
    ncases = 0;
     ...
     if (parents.elementAt(i).getTypeOfVariable() ==1) {
      ...
      if (var.getName().contains("ias")) {
        // System.out.println("ncases " + ncases + "totalparents " + totalparents);
     }
      
     freq = new double[ncases][totalparents];
      ...
      for(i=1 ;i< getNumberOfCases(); i++){
         values =  getCase(i);
         valuesp = getCase(i-1);
       //  System.out.println(values[0]);
          factor = 1;
         indexparents = 0;
         valuevar = values[this.getVariables().indexOf(var)];
         
   //      System.out.println("Valor de la variable en caso " + i + " = " + valuevar);
         for(j=parents.size()-1;j>=0;j--) {
  //           System.out.println(parents.elementAt(j).getName());
   //            System.out.println("Lugar variables " + this.getVariables().indexOf(parents.elementAt(j)));    
  //                   System.out.println(valuesp[this.getVariables().indexOf(parents.elementAt(j))]);    
             indexparents += factor*valuesp[this.getVariables().indexOf(parents.elementAt(j))];
             factor = factor  * size[j];
         }
    //        System.out.println("In dexParents" + " = " + indexparents);
         freq[valuevar][indexparents]++;
      }
...
public double[][] getFrequencies2 (Node var,NodeList parents, double[] change) {
    
    double[][] freq;
    int ncases,i,j;
    int [] values,valuesp;
    int valuevar,valueparent;
    int indexparents;
    int totalparents;
    int factor;
    int[] size;
   ...
   //                  System.out.println(valuesp[this.getVariables().indexOf(parents.elementAt(j))]);    
             indexparents += factor*valuesp[this.getVariables().indexOf(parents.elementAt(j))];
             name = parents.elementAt(j).getName();
             if (var.getName().equals(name)) {
                 if(valuesp[this.getVariables().indexOf(parents.elementAt(j))] == values[this.getVariables().indexOf(var)]){
                     equal = true;
                 }
             }
             factor = factor  * size[j];
