...
import elvira.inference.elimination.QualitativeVariableElimination;
import elvira.*;
...
public PairTable(IDiagram diag,Evidence evidence){
  ...
  Relation relation;
  IDiagram diagCopy;
  QualitativeVariableElimination eval;
  ...
  eval=new QualitativeVariableElimination(diag,false);
...
public Node nextToRemoveCM() {
  
  NodePairTable node;
  int i;
  double s, min = 90.0E40;
  FiniteStates  var2;
  Node var;
  ...
  for (i=0 ; i<info.size() ; i++) {   
    ...
    if (node.relations.size() == 1)
      return node.variable;
      var2=(FiniteStates) node.getVariable();
    s = node.totalSize()/var2.getNumStates();
