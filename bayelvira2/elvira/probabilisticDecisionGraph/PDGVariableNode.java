...
public class PDGVariableNode{
	...
	PDGParameterNode getParameterRootNode(){
		...
		if(predecessor == null && parameterNodes.size() == 0){
	...
	int countIndependentParameters(){
		int c = parameterNodes.size() * (var.getNumStates() - 1);
		for(PDGVariableNode y : successors)
	...
	boolean insertMultipleEvidence(elvira.Configuration conf) throws PDGIncompatibleEvidenceException {
	...
	void removeParameterNode(PDGParameterNode p){
		...
		if(idx == -1){
			...
			for(PDGParameterNode p : parameterNodes){
				...
				for(int h = 0;h<this.var.getNumStates();h++){
					...
					succp.updateValuesFromReach(0.0);
				}
			}
		}
		return idx;
	...
	void updateValues(double pc){
		updateValues(pc, false);
	}
	
	void updateValues(double pc, boolean recomputeCounts){
		for(PDGParameterNode p : parameterNodes){
