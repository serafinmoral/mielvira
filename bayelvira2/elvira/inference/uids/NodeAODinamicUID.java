...
public class NodeAODinamicUID extends NodeAOUID {
	...
	for (int i=0;i<allDescendants.size();i++){
		...
		auxInstantUtilRels = instantiateRelations(auxNodeGSDAG.utilityRelations);
		auxInstantProbRels = instantiateRelations(auxNodeGSDAG.probabilityRelations);
...
/*	@Override
	...
	protected Potential calculateConditionedProbabilities() {
		...
		instantiatedProbRels = instantiateRelations(nearestDescsEvaluated.get(0).probabilityRelations);
