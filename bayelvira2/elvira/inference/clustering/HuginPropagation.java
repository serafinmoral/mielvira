...
Relation locateSmallestTable(NodeList interest) {
  ...
  // first we initialize the relation 

  r = new Relation();
 

  // now we locate a clique containing vars
  ...
  for (i=0 ; i<s ; i++) {
    ...
    included = true;
    for (j=0; j<interest.size(); j++ ){
        if (vars.getId(interest.elementAt(j).getName()) == -1) {included=false;}
    }
    if (included) { // fs is included in this clique
      r = njt.getNodeRelation(); 
     return r; 
    }
...
/**
 * Gets the posterior distribution for variables <code>vars</code>
 * It is assumed that vars are included in a clique. In other case it
 * computes a 0 potential.
 */

public PotentialTable getPosterior(NodeList vars){
   
    PotentialTable rp;
    Potential pot;
   Relation r;
   
   rp = new PotentialTable(vars);
   
   
   
    r = locateSmallestTable(vars);
    if (r.getVariables().size() >0) {
             pot = r.getValues();
             pot = pot.marginalizePotential(vars.toVector()); 
   
            pot.normalize();

            if (pot.getClassName().equals("PotentialTree")) {
              rp = new PotentialTable((PotentialTree)pot);
            } 
             else { rp = (PotentialTable)pot; }
      
    }
    else {rp.setValue(0.0);}
...
public void propagate(NodeJoinTree root, String divide) {
 ...
 // System.out.print("Entrando en propagar");
