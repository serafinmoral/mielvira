...
public class BinaryProbabilityTree {
   ...
   static private void getTreeFromPotential(Potential pot, BinaryProbabilityTree tree,
      ...
      if (varToExplore == pot.getVariables().size()) // if all the variables have been explored
      {
         ...
         if (lenLeftListStates == 1) {          
            ...
            System.out.println("Error in BinaryProbabilityTree.getTreeFromTable():" +
   ...
   /**
    * Constructs a <code>BinaryProbabilityTree</code> from a Potential of 
    * any class. The new tree is build so that the more informative variables are
    ...
    * Potential that <code>pot</code>
    */
  /* BinaryProbabilityTree getSortedTreeFromPotential(Potential pot) {
      long treeSize = pot.getSize();    
      int nVars = pot.getVariables().size();
      ...
      for(int i=0;i<nVars;i++){
         varArray[i] = (FiniteStates)(pot.getVariables().get(i));
         listStatesVector[i]=new ListStates( varArray[i] );
      }   
      BinaryProbabilityTree unsortedTree = getTreeFromPotential(pot);  
      getSortedTreeFromBinaryPT(unsortedTree,treeSize,varArray,listStatesVector,
              tree);
      return tree;   
   }*/
