...
import elvira.*;
import elvira.gui.EditVariableDialog.MiActionListener;
 ...
 * <code>ContinuousProbabilityTree</code>. A continuous probability tree is a
 * compact representation of a probability function which depends on continuous
 * and discrete variables. Each internal node represents a variable (continuos
 * or discrete) and each leaf node represents a
 * <code>MixtExpDensity</code>. Each discrete variable node has as many children
 * as possible values it has and each continuous variable has an associated
 * vector of cut points and as many children as the size of this vector minus 1.
 * Each child
 * <code>i</code> corresponds to the interval limited by the cut point
 * <code>i</code> and cut point
 * <code>i+1</code>. The density stored in a leaf corresponds to the probability
 * of the configuration that leads from the root node to that leaf.
 *
 * @since 30/7/2013
 * @version 1.59
 */
public class ContinuousProbabilityTree implements Serializable {
    ...
    Vector child;
    /**
     * Vector of cut-points. They define the intervals in which the domain of
     * the variable is partitioned. There will be one sub-interval for each
     * child of this node.
     */
    Vector cutPoints;
    /**
    ...
    long leaves;
    /**
     * Number of exponential terms in which the continuous variables of the tree
     * are splitted
     */
    int numSplits;
    /**
    ...
    /**
     * Creates a tree with the argument (a discrete variable) as root node and
     * assigning the values given as parameter
     *
     * @param variable a <code>FiniteStates</code> variable.
    ...
    /**
     * Creates a tree with the argument (a continuous variable) as root node and
     * all its children being equal to a constant value given as argument.
     *
     * @param variable a <code>Continuous</code> variable.
     * @param cp a vector of cut points to determine the children of the
     * continuous variable.
    ...
    /**
     * Creates a probability node with a density that is constant and equal to a
     * given value.
     *
    ...
    /**
     * Creates a Continuous Probability Tree from a discrete Probability Tree.
     *
    ...
    public static void main(String args[]) {
        ...
        Bnet red = new Bnet();
        try {
            red = new Bnet(new FileInputStream("/home/afa/Escritorio/salidaTANnoevi.elv"));
        ...
        for (int i = 0; i < red.getNodeList().size(); i++) {
            ...
            System.out.print("\n" + n.getName() + "\n La media es: ");
            if (n instanceof Continuous) {
                System.out.print(((PotentialContinuousPT) red.getRelation(n).getValues()).getTree().firstOrderMoment());
            }
...
//        cpt.print();
     ...
     */
    /**
     * Determines whether a variable is in the tree starting in this node or
     * not.
     *
     * @param variable a <code>FiniteStates</code> variable.
     ...
     */
    public boolean isIn(Node variable) {
     ...
     * @param intervals Number of intervals as maximum when splitting the
     * domain.
     * @param numpoints Number of points used for each line in the derivative
     * method
     *
     *
    ...
    /**
     * This method splits the domain of the continuous variable in several
     * intervals satisfying this two conditions: No changes in concavity and no
     * changes in increas/decrease. We give a parameter indicating the maximun
     * number of intervals so that we join those neighbour intervals with a
     * minimun distance in terms of increase/decrease and concavity.
     *
     * This method will have three differents parts: First we will split the
     * domain in terms of increase/decrease. Later we will split each of the
     * intervals obtained before in terms of changes in concavity/convexity, and
     * at last we will join the intervals if necessary so that there are only
     * <code>intervals</code> splitting the domain.
     *
    ...
    public static Vector domainSplitting(Vector x, Vector y, int intervals) {
        ...
        increase = 0;
        if (((Double) y.elementAt(0)).doubleValue() < ((Double) y.elementAt(1)).doubleValue()) {
            increase = 1;
        }
        ...
        for (i = 1; i < (x.size() - 1); i++) {
            ...
            increaseNow = 0;
            if (((Double) y.elementAt(i)).doubleValue() < ((Double) y.elementAt(i + 1)).doubleValue()) {
                increaseNow = 1;
            }

            if (((Double) y.elementAt(i)).doubleValue() == ((Double) y.elementAt(i + 1)).doubleValue()) {
                increaseNow = increase;
            }
    ...
    public int getNumberofLeaves() {
        ...
        if (this.isProbab()) {
            ...
            for (int i = 0; i < this.getNumberOfChildren(); i++) {
                tam = tam + this.getChild(i).getNumberofLeaves();
            }
            return tam;
        }
    }

    public void divideBranchesIntoPositiveAndNegatives(ContinuousProbabilityTree tree, Continuous yi) {

        if (tree.getVar() != null) {

            if (tree.getVar().equals(yi)) {
                for (int i = 0; i < tree.getNumberOfChildren(); i++) {
                    //interval greater than 0
                    if (tree.getCutPoint(i) < 0.0 && tree.getCutPoint(i + 1) > 0.0) {
                        tree.getChilds().insertElementAt(new ContinuousProbabilityTree(0.0), i + 1);
                        tree.getCutPoints().insertElementAt(new Double(0.0), i + 1);
                    }
                }
            } else {
                for (int i = 0; i < tree.getNumberOfChildren(); i++) {
                    divideBranchesIntoPositiveAndNegatives(tree.getChild(i), yi);
                }
            }
        }
    }
    ...
    /**
     * Assigns a discrete variable to a tree. It creates a vector of children of
     * size equal to the number of cases of the discrete variable.
     *
     ...
     *
     * @return <code>true</code> if the node is a density and <code>false</code>
     * otherwise.
     */
     ...
     *
     * @return <code>true</code> if the node is a variable (discrete or
     * continuous) and <code>false</code> otherwise.
     */
     ...
     *
     * @return <code>true</code> if the node is empty and <code>false</code>
     * otherwise.
     */
    ...
    /**
     * Sets the number of splits in which the domain of the continuous variable
     * are splitted.
     */
     ...
     *
     * @param i an int value. Number of child to be returned. (first value *
     * is <code>i=0</code>).
    ...
    /**
     * Replaces a
     * <code>ProbabilityTree</code> as child of this node. It is inserted at the
     * specified position in the vector of children. The previous child at that
    ...
    /**
     * Gets the position of the cutpoint corresponding to a given value.
     *
     * @param x the value to locate.
     * @return the position of the cutpoint containing <code>x</code>, and -1 if
     ...
     * configuration <code>conf</code> and restricted to the values of that
     * configuration.
     */
        ...
        if (isDiscrete()) { // If the node is a discrete variable
            ...
            if (isContinuous()) {
                ...
                if (xval >= inf) {
                    ...
                    return (new MixtExpDensity(1E-09));
                }
                if (index > -1) {
    ...
    /**
     * Assigns a density to a node. The density is not duplicated. Also, sets
     * the label to PROBAB_NODE.
     *
    ...
    /**
     * Combines a density with a Continuous tree. It multiplies the densities.
     ...
     * with <code>f</code>.
     * @param flag an integer indicating if the MixtExpDensities must be
     * simplified
    ...
    /**
     * Combines two Continuous trees. It multiplies the associated densities. To
     * be used as a static function.
     *
    ...
    /**
     * Combines two Continuous trees. The second tree is to be restricted to the
     * configuration
     * <code>c1</code> and to the configuration of intervals
     * <code>c2</code>. It is an auxiliar function to
     * <code>combine(ContinuousProbabilityTree tree1,ContinuousProbabilityTree tree2)</code>.
     * It multiplies the associated densities. To be used as a static function.
     *
     ...
     * <code>f</code>.
     * @return a new <code>ContinuousProbabilityTree</code> resulting from
     * adding <code>f</code> and <code>tree</code>.
     */
    ...
    /**
     * Sums two continuous trees. It sums the associated densities. To be used
     ...
     * <code>tree1</code>.
     * @return a new <code>ContinuousProbabilityTree</code> resulting from
     * adding <code>tree1</code> and <code>tree2</code>.
     */
    ...
    /**
     * Sums two continuous trees. The second tree is to be restricted to the
     * configuration
     * <code>c1</code> and to the configuration of intervals
     * <code>c2</code>. It is an auxiliar function to
     * <code> combine(ContinuousProbabilityTree tree1,
     *				      ContinuousProbabilityTree tree2) </code> It sums the associated
     * densities. To be used as a static function.
     *
    ...
    /**
     * It removes a discrete variable from a continuous probability tree by
     * summing over it.
     *
    ...
    /**
     * It removes a continuous variable from a continuous probability tree by
     * integrating with respect to it.
     *
     * @param variable the variable to be removed.
     * @return the continuous probability tree result of the deletion.
     */
     ...
     *
     * @param variable the continuous variable with respect to which the
    ...
    /**
     * It restricts a tree to a discrete configuration and to a configuration of
    ...
    /**
     * It restricts a tree to a continuous configuration containing values for
     ...
     * <code>MixtExpDensity</code>). The result is that the leaves are
     * normalized to get that the integral (sum) over the whole domain of the
     * continuous variable is equal to 1. The object is modified.
     *
     * @param flag An integer indicating if the MixtExpDensities in the leaves
     * must be simplified
     */
    ...
    /**
     * Puts
     * <code>copt</code> as a child of the ContinuousProbabilityTree in the
     * position
     * <code>i</code> of the vector child.
     *
    ...
    public void setNewChildren(int numChildren) {
        this.child = new Vector();
        for (int i = 0; i < numChildren; i++) {
            this.child.addElement(i);
        }
    }

    /**
     * Assign a vector of cutpoints to the current variable of the tree. TAKE
     * CARE! this method can only be used if the CPT is constructed manually
     *
     * @param vCutPoints
     */
    public void setCutPoints(Vector<Double> vCutPoints) {
        this.cutPoints = vCutPoints;
    }

    /**
     * It assumes that the node is continuous and sets all the children to value
     * 0.0. The object is modified.
     */
    ...
    /**
     * This method operates over a continuous variable whose children are
     * probability nodes (
     * <code>MixtExpDensity</code>). The result is that the independent term in
     * the selected leaf is incremented in a given value. The object is
     * modified.
     *
     ...
     *
     * @param p the <code>PrintWriter</code> where the tree will be written.
     * @param j a tab factor (number of blank spaces before a child is written).
     */
    ...
    /**
     * Reads data from a file and puts it into a vector. It is necessary just
     * for experiments from the command line
     *
    ...
    /**
     * This method transforms a vector of reals in two vectors, one with the
     * values of the variable, and another one with the frecuencies.
     *
     *
     * @param data Vector containing the data.
     * @param longitud Length of the intervals used to get the empirical
     * density.
     *
    ...
    /**
     * This method transforms a vector of reals in two vectors, one with the
     * values of the variable, and another one with the frecuencies. It makes
     * the same as
     * <code>empiricDensity</code>
     *
     *
    ...
    /**
     * This method transforms a vector of reals in two vectors, one with the
     * values of the variable, and another one with the frecuencies. It divides
     * the interval in
     * <code>intervals</code> each of them with equal number of points on them.
     *
     * @param values Vector containing the data.
    ...
    /**
     * This method sorts a vector using the method bubble. It does not return
     * anything, it changes the elements positions of the vector.
     *
    ...
    /**
     * This method will prune the tree according to some parameters. We will
     * prune just leaves of the tree, that is, the probability values. The prune
     * will be done in terms of deleting some exponential term that countls less
     * than a delta% of the total mass of the function. Also we will prune by
     * joining two branches of a continuous node, or deleting a whole discrete
     * variable by its mean.
     *
     * @param t The tree to prune (It will be modified)
     * @param delta The percentage that must achive an exponential term in order
     * not to be deleted.
     * @param epsilon The error limit to decide if we prune or not.
     * @param conf The configuration that gets to this leave
     * @param tIntegral The total value of the potential. It is used for
     * computing the error.
     *
     */
                            ...
                            if ((mte.factors).size() > 0) {// It has at least on exponential term
                                ...
                                while (i < tam) {
                                    ...
                                    if ((Math.abs(k) / total) < delta) {//We delete, if the error decreases.
                                         ...
                                         *
                                         * z =
                                         * z*(confNew.getUpperValue(d)-confNew.getLowerValue(d));
                                         * }
                                         *
    ...
    /**
     * This method prunes deleting the tree and replacing it by the average of
     * the values All the values must be constant in the children.
     *
     * @param t The tree to prune (It will be modified)
    ...
    /**
     * This method prunes a ContinuousProbabilityTree It tries to apply the
     ...
     * @param conf the configuration of the continuous variables leading to this
     * node
    ...
    public void pruneIterative(ContinuousProbabilityTree t, double delta, double epsilon, double epsilonJoin, double epsilonDisc, ContinuousIntervalConfiguration conf, double tIntegral) {
        ...
        if (X.getTypeOfVariable() == 0) {// The variable is continuous
            ...
            if (t.numberOfMTEChildren() == 0) {// Its children are not probability, I do not prune, I call iteratively to this
                ...
                /**
                 * Lo quito pq no siempre ha de hacer esto, s�lo cuando no es de
                 * la forma X-->f, que es donde falla
                 *
                 * if(nChildren == 1){// There is only one child,
                 *
                 * System.out.println("Estamos en la situacion de mte_pos.
                 * Quitamos la variables y ponemos solo el potencial"); child1 =
                 * t.getChild(0); mte1 = child1.getProb(); t.var = null;
                 * t.assignProb(mte1); t.label = 2; t.cutPoints = null; t.leaves
                 * = 0; }
                 */
    ...
    /**
     * Method for pruning a ContinuousProbabilityTree. This method will call the
     * iterative method.
     *
     * @param t The tree to prune
     * @param delta The threshold
    ...
    /**
     * Computes the error while pruning of two ContinuousProbabilityTree error =
     * int(f(f-g)^2)/(total^3)
     *
    ...
    /**
     * Obtains a ContinuousProbabilityTree representing the marginal density for
     * the variable X.
     *
    ...
    /**
     * This method operates over a ContinuousProbabilityTree whose children are
     * probabilities It returns the value of a X=x, when the MixtExpDensities
     * are defined just for this variable X and the ContinuousProbabilityTree's
     * variable is X
     *
     * @param X the variable
    ...
    /**
     * This method tells you how many of the children are MTE functions not
     * constant.
     *
     * @return An integer
     */
    ...
    /**
     *
     * Returns a nodelist containing all the variables involved in the CPT. It
     * uses getVarIterative
     *
    ...
    /**
     * Return the minimun cut point value for all expanded continuous variables
     * in the tree.
     */
    ...
    /**
     * Return the maximum cut point value for all expanded continuous variables
     * in the tree.
     */
    ...
    public void getListBranches(ContinuousIntervalConfiguration cic, Vector<MixtExpDensity> vMTEs, Vector<ContinuousIntervalConfiguration> vCIC) {

        if (isProbab()) {
            vCIC.addElement(cic.duplicate());
            vMTEs.addElement(this.getProb().duplicate());
            // cic.duplicate().print();
            //System.out.println("METO una MTE");
            //this.getProb().duplicate().print();
        } else {
            for (int i = 0; i < this.getNumberOfChildren(); i++) {
                ContinuousIntervalConfiguration CICchild = cic.duplicate();
                CICchild.putValue((Continuous) this.getVar(), this.getCutPoint(i), this.getCutPoint(i + 1));
                this.getChild(i).getListBranches(CICchild, vMTEs, vCIC);
            }
        }
    }

    /**
     * Removes the positive terms in the MTE densities located in the tree
     *
     * @param tree is the ContinuousProbabilityTree to be updated
     */
    public void removePositiveTerms(ContinuousProbabilityTree tree) {

        if (tree.isProbab()) {
            tree.getProb().removePositiveExponentialTerms();
        } else {
            for (int i = 0; i < tree.getNumberOfChildren(); i++) {
                removePositiveTerms(tree.getChild(i));
            }
        }
    ...
    /**
     * This method completes the tree with new branchs. So, the range of all
     * continuous variables of the tree is included in this tree. If a
     * subinterval is not defined then it's added a new branch with null
     * probability. Example: X1 - (1,2) --> 0.3 - (2,3) --> 0.7 if range(X1) =
     * (0,4). Output: X1 - (0,1) --> 0.0 - (1,2) --> 0.3 - (2,3) --> 0.7 - (3,4)
     * --> 0.0
     *
     */
    ...
    /**
     * This method completes the tree with new branches. So, the range of all
     * continuous variables of the tree is included in this tree. If a
     * subinterval is not defined then it's added a new branch with null
     * probability.
     *
     * @param visitedContinuous, is a Nodelist with the continuous nodes that
     * are zeros expanded in the tree already. At the beginning, this nodelist
     * is empty.
     */
    ...
    /**
     * Creates randomly a fully expanded ContinuousProbabilityTree. The
     * probability values for the discrete variables are simulated by an
     * Exponential distribution, the value of the MTE is simulated as follows:
     * The independent term by an Exponential distribution, the coefficient of
     * the exponential by an Exponential distribution, and the exponent by a
     * Normal distribution. Afterwards the MTE is normalised. Those nodes not
     * being a leaf are splitted according to the parameters.
     *
     * @param parents A vector containing the set of parents.
     * @param var The variable (not conditioning).
     * @param nSplits The number of splits in the range of the continuous
     * variables (1 2, 3, or 4).
     * @param nTerms The number of exponential terms in a leaf (0, 1 or 2).
     * @param meanExpDiscrete The mean of the Exponential distribution the
     * probability of the discrete variables are simulated with.
     * @param meanExpIndep The mean of the Exponential distribution the
     * independent terms are simulated with.
     * @param meanExpcoef The mean of the Exponential distribution the
     * coefficients of the expoenntial terms are simulated with.
     * @param meanNormExponent The mean of the Normal distribution the exponents
     * are simulated with.
     * @param sdevNormExponent The standar deviation of the Normal distribution
     * the exponents are simulated with.
     *
     */
        ...
        if (parents.size() == 0) {// No more parents variable to split with, so I must create the leaf
            ...
            if (var.getTypeOfVariable() == 1) {//It is discrete
                ...
                for (i = 0; i < nSplits; i++) {
                    ...
                    //System.out.println("Num Terms:"+ nTerms);

                    indep = samGen.randomExponential(meanExpIndep);

                    vFact = new Vector<Double>();
                    vCoef = new Vector();

                    for (int j = 0; j < nTerms; j++) {
                        vFact.addElement(samGen.randomExponential(meanExpCoef));
                        vCoef.addElement(samGen.randomNormal(meanNormalExponent, sdevNormalExponent));
                    }
                    /*
                     if(nTerms > 0 ){
                    ...
                    //NUEVO
                    mte = new MixtExpDensity(vFact, vCoef, indep, (Continuous) var);
    ...
    public void removeSubTrees(ContinuousProbabilityTree cpt, Node n) {




        if (!cpt.isProbab()) {
            if (cpt.getVar().equals(n)) {
                cpt.assignProb(0.0);
            } else {
                int children = cpt.getNumberOfChildren();
                for (int i = 0; i < children; i++) {
                    removeSubTrees(cpt.getChild(i), n);
                }
            }
        }
    }
    ...
    /**
     * Computes the mean square error of two Discrete ContinuousProbabilityTree
     * (normalised) error = sum(pi(pi-aproxpi)^2)
     *
     * @param t1 the first tree
     * @param t2 the second tree
     *
    ...
    /**
     * Obtains the number of terms on each leaf, assuming it is constant in
     * every leaf
     *
    ...
    /**
     * Obtains the number of splits on each range of continuous variable,
     * assuming it is constant in every variable
     *
    ...
    /**
     * Obtains the actual size of the potential, that is, the number of
     * exponential terms (including contants) this ContinuousProbabilityTree
     * has.
     *
    ...
    /**
     * This method prunes a tree just eliminating exponentials terms so that
     * remains only two exponential terms. This method is an interative method
     * that tries to localize where the leaves are
     *
     * @return (int) The number of splits
     */
    ...
    /**
     * This method prunes a tree eliminating exponentials terms whose shape are
     * quite similar to a constant function.
     *
     * This method is an iterative method that tries to localize where the
     * leaves are
     *
     */
    public void pruneConstantTerms(ContinuousIntervalConfiguration conf, double threshold) {
        ...
        if (isProbab()) { // Es una hoja, asi que hay que le pasamos el metodo de MTE
            ...
            mte = getProb();
            System.out.println("MTE antes de podar");
            mte.print();

            mte2 = mte.pruneConstantTermsLeaf(conf, threshold);
            System.out.println("MTE despues de podar");
            mte2.print();
            ...
            for (int i = 0; i < getNumberOfChildren(); i++) {
                ...
                child = getChild(i);
                child.pruneConstantTerms(conf, threshold);
    ...
    /**
     * Computes an approximation of the Kullback-Leibler divergence from this
     * mixed tree to a given one. The integral is approximated by a sum of as
     * many rectangles as given. Both trees are supposed to be defined over the
     * same variable, and only one.
     *
     * @param t the <code>ContinuousProbabilityTree</code> with respect to which
     * the KL divergence will be computed.
    ...
    public double firstOrderMomentSimulate() {
        ...
        for (i = 0; i < vecMin.size(); i++) {
            ...
            //  if (flag){ //Integracion por MonteCarlo
            //   System.out.println("Integración por montecarlo");
            double integral = 0.0;
            for (int it = 0; it < 5000; it++) {
                double u = Math.random() * (max - min) + min;
                integral = integral + u * mte.getValue((Continuous) this.getVar(), u);
                //System.out.println(mte.getValue((Continuous)this.getVar(), u));
            }
            integral = integral * (max - min) / 5000.0;
            res = res + integral;

//            else{ //Integracion normal
//
...
//            }
    ...
    public double firstOrderMoment() {
        ...
        for (i = 0; i < vecMin.size(); i++) {
            ...
            //Compruebo si algún b es <0.01
            boolean flag = false;
            for (int k = 0; k < mte.getNumberOfExp(); k++) {
                LinearFunction l = ((LinearFunction) mte.getExponent(k));
                b = l.getCoefficient(0);
                if (Math.abs(b) < 99999999999999999999999990.01) {
                    flag = true;
                }
            }
        ...
        /* res = res + (mte.getIndependent()*(((max*max)-(min*min))/2));
         ...
         if (Math.abs(b)<10000){//Integracion MonteCarlo
         double integral = 0.0;
                   
         for (int it=0;it<100;it++){
         double u = Math.random()*(max-min) + min;
         integral = integral + u * a*Math.exp(u*b);
         }
         integral = integral *(max-min)/100.0;
         res =res+integral;
...
//            max = ((Double)vecMax.elementAt(i)).doubleValue();
//            
//            double incr = (max - min)/nit;
//            
//            
//            //Compruebo si algún b es <0.001
//                 double integral = 0.0;
//                 double u = min;
...
//                      u = u + incr;
//                 }
//                 integral = integral *(max-min)/(double)nit;
//                 res =res+integral;              
//        }
//        return res;        
//    }
//    
    /**
     * Computes the quantiles of the distribution. It assumes that the tree is a
     * density, i.e., integrates up to one. Actually, the method returns an
     * approximation with error lower than 0.01
     *
    ...
    /**
     * Computes the median of the distribution. It assumes that the tree is a
     * density, i.e., integrates up to one. Actually, the method returns an
     * approximation with error lower than 0.01
     *
    ...
    public Vector getMaxMinMTEsVector() {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (child.isProbab()) {
                ...
                // System.out.println("El árbol tiene "+cutPoints.size()+" cutpoints");
    ...
    /**
     * Simulates a value for the variable in this node. The node must be a
     * variable (discrete or continuous) and all of its children must be
     * probability nodes. The MTE must be normalized
     *
...
//	System.out.println("****");
     ...
     * @param p4 = P(Xi|C)
     *
     * @return the conditional mutual information between both variables
     * �(Xi,Xj|C)
     *
     */
    ...
    /**
     * Replaces a variable by a linear function. It requires that the exponents
     * are linear functions. The object is modified.
     *
     * @param v the variable (Continuous) to replace.
     * @param lf the LinearFunction that will replace variable <code>v</code>.
     * @return a new density, where <code>v</code> is replaced *
     * by <code>lf</code>.
     */
    ...
    /**
     * Multiplies all the leaves by a given constant. The object is modified.
     *
    ...
    /**
     * Gets the list of intervals compatible with a configuration.
     *
     * @param conf a configuration
     * @return the list of intervals, as *
     * a <code>ContinuousIntervalConfiguration</code>
     */
    public ContinuousIntervalConfiguration getIntervals(ContinuousConfiguration conf) {

        ContinuousIntervalConfiguration intervals = new ContinuousIntervalConfiguration();

        if (isDiscrete()) {
            int pos = conf.indexOf(var);
            if (pos == -1) {
                System.out.println("ERROR getting the intervals: missing discrete variable " + var.getName());
            } else {
                int element = conf.getValue(pos);
                intervals = intervals.merge(((ContinuousProbabilityTree) child.elementAt(element)).getIntervals(conf));
            }
        } else {
            int pos = conf.getIndex((Continuous) var);
            if (pos == -1) {
                System.out.println("ERROR getting the intervals: missing continuous variable " + var.getName());
            } else {
                double x = conf.getContinuousValue(pos);
                int s = child.size();

                for (int i = 0; i < s; i++) {
                    double x1 = ((Double) cutPoints.elementAt(i)).doubleValue();
                    double y1 = ((Double) cutPoints.elementAt(i + 1)).doubleValue();
                    if ((x >= x1) && (x <= y1)) {
                        intervals.putValue((Continuous) this.getVar(), x1, y1);
                        intervals = intervals.merge(((ContinuousProbabilityTree) child.elementAt(i)).getIntervals(conf));
                    }
                }
            }
        }
    ...
    /**
     * Updates a tree by branching at a continuous variable.
     *
     * @param v the variable where the split will be done
     * @param vals the correct values to put in the leaves.
     * @param conf the configuration corresponding to the branch to split.
     */
    public void updateTree(Continuous v, double val, ContinuousConfiguration conf) {

        Node n = this.getVar();

        if (this.isProbab()) {
            this.value = new MixtExpDensity(val);
        }

        if (n == v) {
            int s = this.child.size();
            double valueToSplitBy = conf.getValue(v);
            int i = 0;
            boolean found = false;
            while ((i < s) && (!found)) {
                double x1 = ((Double) cutPoints.elementAt(i)).doubleValue();
                double y1 = ((Double) cutPoints.elementAt(i + 1)).doubleValue();
                if ((valueToSplitBy >= x1) && (valueToSplitBy <= y1)) {
                    double newSplitValue = (x1 + y1) / 2;
                    cutPoints.add(i + 1, new Double(newSplitValue));
                    found = true;
                    ContinuousProbabilityTree newTree = this.getChild(i).copy();
                    if (valueToSplitBy < newSplitValue) {
                        this.child.add(i + 1, newTree);
                        this.getChild(i).updateTree(v, val, conf);
                    } else {
                        this.child.add(i, newTree);
                        this.getChild(i + 1).updateTree(v, val, conf);
                    }
                }
                i = i + 1;
            }
    ...
    /**
     * Updates a tree by branching at a discrete variable.
     *
     * @param v the variable where the split will be done
     * @param vals the correct values to put in the leaves.
     * @param conf the configuration corresponding to the branch to split.
     */
    public void updateTree(FiniteStates v, double val, ContinuousConfiguration conf) {
        ...
        if (n == v) {
            int s = this.child.size();
            int valueToSplitBy = (int)conf.getValue(v);
            for (int i=0 ; i<s ; i++) {
                if (i == valueToSplitBy) {
                    this.getChild(i).updateTree(v, val, conf);
                }
