...
public class ProbabilityTree {
   ...
   public  ProbabilityTree(ProbabilityTree t,Configuration cv) {
    ...
    public ProbabilityTree getLeaf(Configuration conf) {
    ...
    public void update(Hashtable positions,  int[] conf, double newVal, NodeList aux, int mode){
      ...
      public void update2(Configuration c, double newval,Vector aux){
      ...
      public void update3(Configuration c, Vector listap,Configuration cv, FiniteStates n){
        ...
        if (label == FULL_NODE) {
            ...
            tree = (ProbabilityTree)child.elementAt(val);
            cv.insert(var,val);
            tree.update3(c,listap,cv,n);
        }
        else if (label == PROBAB_NODE) {
           tree =  ((PotentialTree) listap.elementAt(0)).getTree().restrict(cv);
           for(i=1;i<listap.size();i++) {
               tree = ProbabilityTree.combine(tree, ((PotentialTree) listap.elementAt(i)).getTree().restrict(cv) );
           }
           
          tree =  tree.maximizeOverVariable(n);
      ...
      public void upgradeList(Configuration cv, Configuration c) {
          int val;
          ProbabilityTree tree;
      ...
      public ProbabilityTree createTree(Configuration c, Vector listap,Configuration cv, FiniteStates n){
    ...
    /**
     * Computes the addition of all the values to the square in the tree starting
     * in this node.
     * @param treeSize size of the completely expanded tree.
     * @return the addition computed.
     */
    
    public double sumsquare(long treeSize) {
...
//        print(4);
    ...
    /**
     * Bounds the tree by substituting nodes whose children are
     * leaves by the maximum of them. This is done for nodes
     * with an information value lower than a given threshold.
     * @param limit the information threshold for pruning.
     * @param oldSize size of this tree if it were complete.
     * @param globalSum the addition of the original potential 
     * @param globalSum2 the addition of the squares of values of the original global potential
     * @param method the method used to compute the information
     * @param numberDeleted an array with a single value storing
     * the number of deleted leaves.
     * @return <code>true</code> if the tree has been reduced to a probability
     * node; <code>false</code> otherwise.
    ...
    public boolean maxprune(double limit, int method) {
        long Oldsize;
        double s[];
        double s2[];
        long nd[];
        ...
        Oldsize = (long) getVarList().getSize();
        s[0] = sum(Oldsize);
        s2[0] = sumsquare(Oldsize);
        nd = new long[1];
        
        nd[0]=0;
        
          
        return(maxprune(limit,Oldsize,s,s2,method,nd));
    }
    
     
 public boolean pruneEqual() {
         boolean bounded,childBounded;
         int i,nv;
         ProbabilityTree ch;
         double  vs;

         if(this.isProbab()) {return true;}
         nv = var.getNumStates();
      //   System.out.println("Tree var" + var.getName());
        ch = getChild(0);
        bounded = ch.pruneEqual();
        vs=1.0;
        if (bounded) {vs = ch.value;}

        for (i=1 ; i<nv ; i++) {
            ch = getChild(i);
            
            childBounded = ch.pruneEqual();
            if(childBounded && bounded) {
                if (vs!=ch.value){
                    bounded=false;
                }
            }
    ...
    public void recursiveSplit(ProbabilityTree free,ProbabilityTree
core, FiniteStates vars) {
        ProbabilityTree core2;
        int i,nv;

        if (this.isProbab()) {
            free.assignProb(value);
            core.assignProb(1.0);
        ...
        if(this.var ==vars){
            free.assignProb(1.0);
             nv= this.var.getNumStates();
            core.assignVar(this.var);
             for(i=0;i<nv;i++){
              core.setNewChild(this.getChild(i), i);
        }
        }
        else{
         nv= this.var.getNumStates();
         free.assignVar(this.var);
         core.assignVar(this.var);
         for(i=0;i<nv;i++){
              this.getChild(i).recursiveSplit(free.getChild(i),core.getChild(i),vars );
        }
        }
    ...
    public boolean maxprune(double limit, long oldSize, double globalSum[], double globalSum2[], int method,
    long numberDeleted[]) {
        ...
        int i, numberChildren;
        ProbabilityTree ch;
        double pr, sum = 0.0, entropy = 0.0, info, sum2=0.0;
        boolean bounded = true, // tell if the tree can be reduced to a probab. node
        childBounded;
        
       // System.out.println("Entro de Max Prune") ;
        
        
        if(label== PROBAB_NODE) {return(true);}
        else {
        ...
        for (i=0 ; i<numberChildren ; i++) {
            ...
            if (ch.label == PROBAB_NODE) {
                pr = ch.value;
                 if(method==0) { sum += pr; entropy += (-pr * Math.log(pr));}
                 if(method==1) {sum2 += pr*pr; sum += pr;}
                   if((method==2)|(method==3)|(method==4)|(method==5)) { sum += pr;}
                
            }
            else {
                long chOldLeaves=ch.leaves;
                childBounded = ch.maxprune(limit,newSize,globalSum,globalSum2,method,numberDeleted);
                leaves-=(chOldLeaves-ch.leaves);
                if (!childBounded)
                    bounded = false;
        ...
        if (bounded) {
            ...
            if (sum > 0.0) {
                ...
                switch(method) {
                    case 0: {info = ((newSize * sum)) *
                            (Math.log(numberChildren) - Math.log(sum)) - newSize*entropy;
                             info=info/globalSum[0];
                             break;
                    }
                     case 1: {   double x = globalSum2[0];
                                 pr = maximumValue();
                                 info = Math.log(x+newSize*(pr*pr*numberChildren-sum2)) - Math.log(x);
                                 break;
                    }
                       case 2: {   double x = globalSum[0];
                                 pr = maximumValue();
                                 info = Math.log(x+newSize*(pr*numberChildren-sum)) - Math.log(x);
                                 break;
                    }
                        case 3: {   double x = globalSum[0];
                                 pr = maximumValue();
                                 info = (newSize*sum/x) *(Math.log(pr*numberChildren) - Math.log(sum));
                                 break;
                    }
                        case 4: {   double x = globalSum[0];
                                 pr = maximumValue();
                                 info = Math.log(pr*numberChildren) - Math.log(sum);
                                 break;
                    }
                         case 5: {   
                                 
                                 
                                 info = -minimumValue()/maximumValue();
                                 break;
                    }
        ...
        //   System.out.println("Info " + info);
            if (info <= limit) {
             //    System.out.println("Prunning " + limit + " Info " + info);
                pr = maximumValue();
             //   System.out.println(pr*numberChildren+ " " + sum);
                numberDeleted[0] += numberChildren-1;
                assignProb(pr);
                child = new Vector();
                if ((method==0)||(method==2)||(method==3)||(method==4)||(method==5)) {globalSum[0] += newSize * (numberChildren*pr-sum);}
                if (method==1) {globalSum2[0] += newSize * (numberChildren*pr*pr-sum2);
                                globalSum[0] += newSize * (numberChildren*pr-sum);}
        ...
        // System.out.println("Salgo de Max Prune") ;
        return bounded;
    }
    }
...
public int isPropApprox(double prI,double prJ,Vector vProbs,Vector vfacK,
  ...
  return (indexFac); // returns 0    (-1 if not proportional)
    ...
    public void log() {
     ...
     /**
     * Multiplies x to the leaves of the tree starting in this node by a value.
     * The object is modified.
     * @param x:  the value to be added.
     *X
     */
