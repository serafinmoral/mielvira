...
//        System.out.println("generar pt desde rpt");
        PotentialTable aux = ((RecursivePotentialTree)pot).RPTtoPotentialTable();
        this.variables = aux.getVariables();
...
public PotentialTree restrictVariabletoTree(Configuration conf) {
  ...
  setTreeFromTable(pot.getTree(),conf,aux);
...
public PotentialTable maxVariable(Vector vars) {
  ...
  for (i=0 ; i<values.length ; i++) {
    auxConf2 = new Configuration(auxConf1,vars);
    pos = auxConf2.getIndexInTable();
    if (values[i]> pot.values[pos])  pot.values[pos]= values[i];
    auxConf1.nextConfiguration();
  }
...
public void setTreeFromTable(ProbabilityTree tree,
  ...
  if (vars.size() == 0) {
    ...
    for(i=0 ; i<var.getNumStates() ; i++) {
      ...
      tree.leaves += tree.getChild(i).leaves;
...
/**
 * multiply a quantity by all the elements of a potential.
 * 
 */
@Override
public Potential mul(double x) {
  Potential aux = new PotentialTable(this.values.length);
  aux.setVariables(this.variables);

  for (int i=0 ; i<values.length ; i++){
    ((PotentialTable)aux).setValue(i, x*this.getValue(i));
  }
  return aux;
}

/**
 * divide all the elements of a potential by a number.
 *
 */
@Override
public Potential div(double x) {
  Potential aux = new PotentialTable(this.values.length);
  aux.setVariables(this.variables);

  for (int i=0 ; i<values.length ; i++){
    ((PotentialTable)aux).setValue(i, this.getValue(i)/x);
  }
  return aux;
}
...
public static int getIndexInTable(int[] values, double[] weights){
  ...
  /**
   * changes this potential's values by 1/value
   */

  public Potential invert() {

      PotentialTable nuevo = new PotentialTable(this);
      for(int k=0; k<this.values.length; k++){
          nuevo.setValue(k, (1/this.getValue(k)));
      }
