...
public class learningRPT_DB2 {
    ...
    */
   public learningRPT_DB2(/**DataBaseCases dat*/){
       //cases = dat;
   }
...
//System.out.println("Score between "+ vars.get(i).getName() + " and "+vars.get(j).getName() +" cond: "+ condScore + " partial: "+ partialScore);
               if (score > 0) {
System.out.println("New link: "+vars.get(i).getName()+" "+ "->"+vars.get(j).getName());
...
System.out.println("Lista de enlaces considerados tras construir grafo.....");
   ...
   public LinkList buildConditionedGraph(DataBaseCases data, Vector<Node> varToCond, Vector<Node> dataVars) {
       ...
       for (int i = 0; i < size; i++) {
           ...
           //Aqui meter el resto de variables de varToCond en varsToCond2
               ...
               for(int k=0; k<varToCond.size(); k++){
                   if (varsToCond2.getId(varToCond.get(k).getName()) == -1)
                       varsToCond2.insertNode(varToCond.get(k));
               }
System.out.println("Test con varsToCond2......");
...
//                }
    ...
    *
    * TambiÃ©n se puede hacer recorriendo la lista de enlaces y sumando a cada variable
    * (cabeza y cola) el peso del link. Esto puede ser mÃ¡s rÃ¡pido
    *
    * @param graph
    * @param vars
    * @return
   ...
   private TreeNode dealWithSplitChain(DataBaseCases data,Vector<Set<Node>> clusters, Vector<Node> y1, Vector<Node> y2){
       TreeNode recTree = null;
System.out.println("----------------------- dealWithSplitChain -------------------");
System.out.print("Variables en y1: ");
for(Node node : y1){
...
//                    TreeNode childTree = factorize(pot);
              ...
              // System.out.println("Cases: "+restricted.getNumberOfCases()+" and vars: "+ restricted.getVariables().toString2());
                   ...
                   restricted.removeVariables(auxrestricted);
System.out.println("Numero de casos en restricted: "+restricted.getNumberOfCases());
                   if(restricted.getVariables().size()>1 && restricted.getNumberOfCases()>0){
System.out.println(" ....... La configuracion a la que se restringe es: ");
conf.pPrint();
System.out.println();
                   }
...
//                Potential pot = potential.marginalizePotential(aux);
    ...
    * entonces el vector sÃ³lo tendrÃ¡ un elemento que serÃ¡ un conjunto con
    * todas las variables del potencial.
    *
    * NOTA:
    * Si el conjunto no posee todas las variables del potencial que se estÃ©
    * analizando es porque no estaban en la lista de enlaces, por lo que
    * cada variable serÃ¡ independiente de las demÃ¡s (grafo no conectado)
    *
    */
   // asc begin 26/3/2010
