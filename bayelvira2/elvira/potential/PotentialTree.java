...
public class PotentialTree extends Potential {
 ...
 // v2 = t.splitOnlyPT(z,factParam);  // BUSCA la z !!
  ...
  Vector statSizeVec= factParam.getClassStatistic(1);
    ...
    public  PotentialTree(Configuration cv,Vector lisp,FiniteStates var ,double x) {
        PotentialTree pt,pt2;
        NodeList nl;
        FiniteStates nod;
        int val;
        ...
        if (lisp.size()>0){
        pt = ((PotentialTree) lisp.elementAt(0)).restrictVariabletoTree(cv);
        
        
        
        for(i=1;i<lisp.size();i++) {
            pt2 = ((PotentialTree) lisp.elementAt(i)).restrictVariabletoTree(cv);
            pt = (PotentialTree) pt.combine(pt2);
        }
        
            varsMax = (Vector) pt.getVariables().clone();
            varsMax.remove(var);
            
           

            pt = (PotentialTree) pt.maxMarginalizePotential(varsMax);
            pt.values.mul(1/x);  
        }
        
        else {
            
            pt = new PotentialTree(new ProbabilityTree(1.0));
        ...
        nl = new NodeList(pt.getVariables());
        
        nl.merge(new NodeList(cv.getVariables()));
        
        variables = (Vector)nl.toVector();
        
        
     
         values = new ProbabilityTree(pt.getTree(),cv);
   ...
   <code>entropyPotential()</code>.
    ...
    public PotentialTree restrictVariabletoTree(Configuration conf) {
        return ((PotentialTree) this.restrictVariable(conf));
    }
    ...
    public Potential combine(Potential p) {
        ...
        double x;
        ProbabilityTree tree, tree1, tree2;
       // System.out.println("Combining tree");
...
/*public PotentialTree combine(PotentialTree p) {
  return (PotentialTree)combine((Potential)p);
   ...
   //     System.out.println("Max marginalizing tree");
    ...
    public void  upgrade(Configuration cv,Vector lisp,FiniteStates var ,double x) {
        PotentialTree pt,pt2;
        NodeList nl;
        FiniteStates nod;
        int val;
        ProbabilityTree leaf;
        Vector varsMax;
        
        int i;
        
        System.out.println("Upgrading tree");
        if (lisp.size()>0){
        pt = ((PotentialTree) lisp.elementAt(0)).restrictVariabletoTree(cv);
        ...
        for(i=1;i<lisp.size();i++) {
            ...
            pt = (PotentialTree) pt.maxMarginalizePotential(varsMax);
            pt.values.maxprune(0,2);
            leaf = this.values.getLeaf(cv);
             pt.values.mul(leaf.value/x);
             leaf = pt.values;
        }
...
public Vector split2(FiniteStates varSplit) {
  ...
  thisTree = this.getTree();
    ...
    freeTerm = new ProbabilityTree();
    coreTerm = new ProbabilityTree();

    thisTree.recursiveSplit(freeTerm,coreTerm,varSplit);

    freeTerm.pruneEqual();
  //  System.out.println("Free term");
 thisTree.print(1);
 System.out.println("Variable " + varSplit.getName());
    ...
    coreTerm.print(1);
      ...
      // *** Converts to Potential Trees ****//
       ...
       // Insert the potential tree without the variable
      PotentialTree pt= new PotentialTree( freeTerm, this.getVariables() );
      i= pt.getVariables().indexOf(varSplit);
      (pt.getVariables()).removeElementAt(i);
      ...
      // Insert the potential tree containing the variable
      listofPT.addElement(new PotentialTree(coreTerm,this.getVariables()));
