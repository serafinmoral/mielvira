...
public class Bnet extends Network implements Serializable{
    ...
    public static void main(String args[]) throws elvira.parser.ParseException, IOException {
        ...
        if (args.length == 2){
            ...
            Bnet  b1 = new Bnet(f1);
            Bnet  b2 = new Bnet();
            b2 = b1.splitNode(b1.getNodeList().elementAt(b1.getNodeList().size()-2));
            b2.saveBnet(f2);
        }
    ...
    public Bnet(String name, int format) throws elvira.parser.ParseException ,IOException{
               super(); 

       BufferedReader file;
       String line;
       int nvar,npot,nvp,nval;
       NodeList listn,listp;
       FiniteStates node;
       Relation rel;
       PotentialTable pot;
        ...
        if (file == null){
         ...
         line = file.readLine(); 
         while ((line.isEmpty())||(line.startsWith("c")) )
         {
                      line = file.readLine(); 
         }
        ...
        line = file.readLine(); 
         ...
         for(int i=0; i<parts.length;i++){
             ...
             node = new FiniteStates(sizes[i]);
        //     System.out.println(sizes[i]);
             node.setName("vars" +i);
             listn.insertNode(node);
         }
         ...
         parts = line.split("(\t| )+");
         nvp = Integer.parseInt(parts[0]);
         listp = new NodeList();
         for(int j=0; j<nvp; j++){
             node = (FiniteStates) listn.elementAt(Integer.parseInt(parts[j+1]));
             listp.insertNode(node);
         }
         rel = new Relation();
         rel.setVariables(listp);
         ...
         while ((line.isEmpty())||(line.startsWith("c")) )
         {
          ...
          nval = Integer.parseInt(line);
          System.out.println(nval);
          
          pot = new PotentialTable(nval);
          pot.setVariables(((Relation) this.getRelationList().elementAt(i)).getVariables().getNodes());
         ...
         while ((line.isEmpty())||(line.startsWith("c")) )
         {
          ...
          for(int j=0; j<parts.length ; j++) {
             // System.out.println(parts[j]);
              pot.setValue(k, Double.parseDouble(parts[j]));
              k++;
          }
          }
          ((Relation) this.getRelationList().elementAt(i)).setValues(pot);
        }
        }
        
//       ((Relation) this.getRelationList().elementAt(511)).print();
    }
    ...
    public double computeDimension() {
        NodeList thenodes, parents;
        double aux,dim=0.0;
        Node var, var2;
        int i,j;
        
        thenodes = getNodeList();
        for(i=0; i<thenodes.size(); i++) {
        
            aux=1.0;
            var = thenodes.elementAt(i);
        //     System.out.println("     Considering Node " + var.getName());
            if (var.getTypeOfVariable()==1){
            aux =  ((FiniteStates) var).getNumStates()-1;
            parents = var.getParentNodes();
            for(j=0; j<parents.size(); j++){
               var2 = parents.elementAt(j);
        //       System.out.println("     Parent " + var2.getName());
               if (var2.getTypeOfVariable()==1){
               aux *=  ((FiniteStates) var2).getNumStates();}
            }
       //         System.out.println("Dimiension " + aux);
 ...
 */
 ...
 public Bnet copyBnet() throws java.lang.Throwable{
 ...
 int i;
  ...
  try{
    ...
    for (int posn=0 ; posn<nodeList.size() ; posn++) {
             co.addRelation(co.getNodeList().elementAt(posn));
           
             pot = getRelation(getNodeList().elementAt(posn)).getValues().copy();
             pot.setVariables( co.getRelation(co.getNodeList().elementAt(posn)).getVariables().getNodes()    );
             co.getRelation(co.getNodeList().elementAt(posn)).setValues(pot);
    }
 ...
 int i;
    ...
    co.nodeList = nodeList;
    
    
    
    for (int posn=0 ; posn< this.getRelationList().size() ; posn++) {
             
             r1= (Relation) this.getRelationList().elementAt(posn);
             r2 = r1.copy();
             co.addRelation(r2);
  ...
  * Returns a copy of this Bnet.
  *
  * @returns a copy of this Bnet. The nodes in the argument are not duplicated.
  */
    
 

    
    public Bnet splitNode(Node var){
 ...
 // System.out.println("Splitting variables " + var.getName());
...
try{
 co = copyBnet();
  ...
  if (var.getTypeOfVariable()==1) {
    ...
    for (int posn=0 ; posn<co.nodeList.size() ; posn++) {
      n = (Node)nodeList.elementAt(posn);
    //  System.out.println("Node " + n.getName());
      if (n.equals(var)) {
        snode = (FiniteStates)co.nodeList.elementAt(posn);
         in = new Integer(snode.getNumStates());
    
        snode.getStates().add(in.toString());
        snode.setNumStates(snode.getNumStates()+1);
        System.out.println("Now this node has " + snode.getNumStates());
        rel = co.getRelationList();   
        for(i=0; i<rel.size(); i++) {
           
            r = (Relation) rel.elementAt(i);
           refnode = r.getNode();
           refold = this.getNodeList().getNode(refnode.getName());
            if (r.isInRelation(snode)) {
              //   System.out.println("considering relation");
             //     r.print();
                  rp = this.getRelation(refold); 
               pot = rp.getValues();
    //           System.out.println("Antiguo potencial");
      //          pot.print();
                
//                for (j=0;j<r.getVariables().size(); j++) 
  //                       {System.out.println("Variable " + r.getVariables().elementAt(j).getName() +
    //                    "Estados " +  ((FiniteStates) r.getVariables().elementAt(j)).getNumStates()); 
      //                  }
              npot = new PotentialTable(r.getVariables());
      //         System.out.println("With New Potential");
        //         npot.print();
             
               conf1 = new Configuration(pot.getVariables());
               conf2 = new Configuration(npot.getVariables());
               nv = (int) r.getVariables().getSize();
             //   System.out.println("number of configurations "+nv);
               for (j=0; j<nv;j++) {
                   
                 
                 //  System.out.println("Configuracion Completa inicial");
               //    conf2.print();
                   
               //    System.out.println();
                   sc=conf2.getValue(snode);
                //   System.out.println("Value of variable " + sc);
                   
                   
                   
                     for ( k=0; k<npot.getVariables().size(); k++) {
                    //     System.out.println("Colocando en variable " + ((Node) npot.getVariables().elementAt(k)).getName() 
                      //           + " El valor " + conf2.getValue( ((Node) npot.getVariables().elementAt(k)).getName()));
                                 
                                 
                         if((sc< snode.getNumStates()-1) || !(((Node) npot.getVariables().elementAt(k)).equals(snode)       )) {
                       conf1.putValue(( (Node) npot.getVariables().elementAt(k)).getName(), 
                               conf2.getValue( ((Node) npot.getVariables().elementAt(k)).getName()));
                //          System.out.println("La configuracion queda como " );
                  //        conf1.print(); System.out.println();
                            }
                         else {
                             
                       conf1.putValue(( (Node) npot.getVariables().elementAt(k)).getName(),  sc-1);
                     }
                     ...
                     //  System.out.println("Configuracion Completa modificada");
                   
                  // conf1.print();  
                     
                   if (sc < snode.getNumStates()-1) {
                       if (r.getNode().equals(snode)) {
                           npot.setValue(conf2, pot.getValue(conf1)*(snode.getNumStates()-1.0)/((double) snode.getNumStates()));
                           conf2.nextConfiguration();
                           
                        }
                       else {
                           npot.setValue(conf2, pot.getValue(conf1));
                           ...
                           if (r.getNode().equals(snode)) {
                           npot.setValue(conf2, (2.0* Math.random())/((double) snode.getNumStates())    );
                           conf2.nextConfiguration();
                           
                        }
                       else {
   
                           conf3 = conf1.duplicate();    
                           x=0;
                           for ( k=0; k<snode.getNumStates()-1; k++) {
                               conf3.remove(var);
                               conf3.insert((FiniteStates) var, k);
                               x+= pot.getValue(conf3);
                           }
                           npot.setValue(conf2, 0.5*x/( (double) snode.getNumStates()-1) + Math.random() );
                           conf2.nextConfiguration();
               ...
               npot.normalizeOver((FiniteStates) npot.getVariables().elementAt(0)); //                System.out.println("Antiguo potencial "); pot.print();
   //        System.out.println("Nuevo potencial "); npot.print();
              ((Relation) co.getRelationList().elementAt(i)).setValues(npot);
                  
 //                  {System.out.println ("Potential " + i + " = " );
  //     ((Relation) co.getRelationList().elementAt(i)).getValues().print();}
 ...
 // for (i=0; i<co.getNodeList().size(); i++) 
 //      {System.out.println ("Variable " + i + " = " + co.getNodeList().elementAt(i).getName() + " Elements " +  ((FiniteStates) co.getNodeList().elementAt(i)).getNumStates()  );}
 ...
 //      ((Relation) co.getRelationList().elementAt(i)).getValues().print();}
   ...
   return co;
    ...
    public Bnet jointNode(Node var){
 ...
 // System.out.println("Joining variables " + var.getName());
  ...
  if (var.getTypeOfVariable()==1) {
    ...
    for (int posn=0 ; posn<co.nodeList.size() ; posn++) {
      ...
      if (n.equals(var)) {
        ...
        snode.getStates().removeElementAt(snode.getNumStates()-1);
        snode.setNumStates(snode.getNumStates()-1);
        System.out.println("Now this node has " + snode.getNumStates());
        rel = co.getRelationList();   
        for(i=0; i<rel.size(); i++) {
  ...
  //                       {System.out.println("Variable " + r.getVariables().elementAt(j).getName() +
    //                    "Estados " +  ((FiniteStates) r.getVariables().elementAt(j)).getNumStates()); 
     ...
     //           System.out.println("number of configurations "+nv);
              ...
              //    conf2.print();
                ...
                //          System.out.println("La configuracion queda como " );
                  //        conf1.print(); System.out.println();
                       ...
                       conf1.putValue(( (Node) npot.getVariables().elementAt(k)).getName(),  sc);
                     }
                   ...
                   if (sc < snode.getNumStates()-1) {
                       if (r.getNode().equals(snode)) {
                           ...
                           conf2.nextConfiguration();  
                            ...
                            conf3 = conf1.duplicate(); 
                               conf3.remove(var);
                               conf3.insert((FiniteStates) var,((FiniteStates) var).getNumStates()-1 );
                               
                     //          System.out.println("Configuration nueva");
                    //           conf2.print();
                     //          System.out.println();
                     //           System.out.println("Configuration antiguas");
                    //           conf1.print();
                    //             conf3.print();
                    //              System.out.println();
                           ...
                           npot.setValue(conf2, pot.getValue(conf1) + pot.getValue(conf3));
                           conf2.nextConfiguration();
                           
                        }
                       else {
   
                          npot.setValue(conf2, (pot.getValue(conf1) + pot.getValue(conf3))/2.0);
                           conf2.nextConfiguration();
         ...
         //           System.out.println("Antiguo potencial "); pot.print();
         //  System.out.println("Nuevo potencial "); npot.print();
                  ((Relation) co.getRelationList().elementAt(i)).setValues(npot);
