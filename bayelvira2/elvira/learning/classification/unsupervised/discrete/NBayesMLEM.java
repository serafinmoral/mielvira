...
///***********************************************
      ...
      {
        dataCased = (double []) sequentialCases.next();
      dataCase = new int[dataCased.length];
        for(k=0;k<dataCased.length;k++)
        { 

          dataCase[k] = (int)     dataCased[k];
 ...
 private double logLikelihood(Vector auxiliarPotentialTables)
  {
   ...
   double[] dataCased;
    ...
    {
      dataCased = (double []) cases.elementAt(i);
      dataCase = new int[dataCased.length];
        for(k=0;k<dataCased.length;k++)
        { 
