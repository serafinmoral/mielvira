...
public class ClusterVar implements Serializable{
  ...
  public ClusterVar(FiniteStates x, NodeList l, DataBaseCases d, int i, double y) {
    classvar = x;
    listvar = l;
    significance = y;
    nvar = l.size();
    ...
    clusterlist = new Vector();
    method = i;
    if (method ==1) {metric = new BDeMetrics(data,x.getNumStates()*2);}
    ...
    for (i=0; i<nvar; i++){
      ...
      for (j=i+1;j<nvar;j++){
        nodej = (FiniteStates) listvar.elementAt(j);
         switch (method){
             case 0 : {degree[i][j] =  1.0 - data.testValue(nodei,nodej,condition);
                       break;}
             case 1 : {degree[i][j] =  significance - metric.scoref(classvar,nodei,nodej);}
         }
...
/*  
  ...
  public void computeDep(){
    ...
    for (i=0; i<nvar; i++){
      for (j=i+1;j<nvar;j++){
          System.out.println(" i: " + i + "j: "+j + "ranking " + ranking[i][j]);
