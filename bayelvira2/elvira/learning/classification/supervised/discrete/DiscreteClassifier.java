...
public abstract class DiscreteClassifier implements Classifier, Serializable{
 ...
 /**
  ...
  public DiscreteClassifier(DataBaseCases data, boolean lap) throws elvira.InvalidEditException{
    ...
    FiniteStates classStates = (FiniteStates)this.cases.getNodeList().lastElement();
...
if (this.classifier.isEmpty())
  throw new ClassifierException(0);
    ...
    for (int i= 0; i< nTest; i++) {
      ...
      if (assignedClass == (int)caselistmem.getValue(i, this.nVariables-1)){
    		accuracy = accuracy + 1;
                 
      }
       {
      this.confusionMatrix.actualize((int)caselistmem.getValue(i, this.nVariables-1), assignedClass);
       this.confusionMatrix.actualize((int)caselistmem.getValue(i, this.nVariables-1));
    }
    }
...
*/
    ...
    for (i= 0; i< nTest; i++) {
      for(j= 0; j< this.nVariables; j++)
    		caseToTest[j] = caselistmem.getValue(i,j);
        int assignedClass = this.assignClass(caseToTest);
        caselistmem.setValue(i, this.nVariables-1, assignedClass);
...
*/
    /***
     * The following line was added by
     */
   ...
   * it uses the result of Pearl (1987).
	 ...
	 * @return a doublevector with a probability associated to each class
   */
  ...
  public double[] classify (Configuration instance){
    ...
    double[] probabilities = new double[classNumber];
