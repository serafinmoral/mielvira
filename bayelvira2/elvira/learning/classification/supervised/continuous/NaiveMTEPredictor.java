...
public class NaiveMTEPredictor {
    ...
    /**
     * Gives an unbiased estimation of the mutual information between the class
     * variable and a feature variable.
     *
     * @param classTree a ContinuousProbabilityTree representing the prior distribution
     * of the class variable.
     * @param featureTree a ContinuousProbabilityTree representing the conditional distribution of
     * a feature variable given the class.
     ...
     */    
    public double estimateMutualInformation(ContinuousProbabilityTree classTree, ContinuousProbabilityTree featureTree, int iterations) {
        
        Node classVar, featureVar;
        double c, f, estimate, px, pf;
        int i;
        ContinuousConfiguration conf, confAux;
        ContinuousProbabilityTree aux, marginalTree;
        MixtExpDensity density;
        
        classVar = classTree.getVar();
        //featureVar = featureTree.getVar();
        
        
        // We compute the marginal for the feature variable.
        
        marginalTree = ContinuousProbabilityTree.combine(classTree,featureTree,0);
        
        marginalTree = marginalTree.addVariable((Continuous)classVar);
        
        estimate = 0.0;
        
        for (i=0 ; i<iterations ; i++) {
            // simulate a value for the class variable.
            c = classTree.simulateValue();
            conf = new ContinuousConfiguration();
            
            conf.insert((Continuous)classVar,c);
            
            // Restrict the conditional tree to the simulated value
            aux = featureTree.restrict(conf);
            featureVar = aux.getVar();
            ...
            confAux.insert((Continuous)featureVar,f);
    ...
    public Vector getListOfMututlinformation() {
        ...
        for (i=0 ; i<n ; i++) {
            if (i != classVariable) {
                ...
                info = estimateMutualInformation(classTree,featureTree,5000);
            
                res.addElement(new Double(info));
            }
            else {
                res.addElement(new Double(-1.0));
            }
        }
    ...
    public static void main(String args[]) throws ParseException,IOException {
        ...
        if (args[1].compareTo("CV") == 0) { // Cross validation
            ...
            double lccMean = 0.0, lccMedian = 0.0, rmseMean = 0.0, rmseMedian = 0.0;
            ...
            for (i=0 ; i<k ; i++) {
                ...
                rmseMean += ((Double)(errors.elementAt(0))).doubleValue();
                lccMean += ((Double)(errors.elementAt(1))).doubleValue();
                
                errors = pred.computeErrors((Vector)results.elementAt(2),(Vector)results.elementAt(3));
                rmseMedian += ((Double)(errors.elementAt(0))).doubleValue();
                lccMedian += ((Double)(errors.elementAt(1))).doubleValue();
            }
            ...
            System.out.println("rmse = "+rmseMean+" ; lcc = "+lccMean);
            
            System.out.println("With the median");
            System.out.println("rmse = "+rmseMedian+" ; lcc = "+lccMedian);
