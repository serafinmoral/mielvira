 ...
 public double score (NodeList vars, Configuration condition){
	...
	
	{
		...
		for(k=0 ; k< nStatesXi ; k++){
		    ...
		    //System.out.println("Su posicion en table: "+pos);
		    Nijk =(int)totalPot.getValue(conf);
		    if (Nijk>0) {sum+=Nijk*Math.log(((double)Nijk)+3.0/nStatesXi -1.0);}
		...
		if(Nij>0){
                partialSum = -Nij*Math.log(((double) Nij)+3.0);}
                else {partialSum = 0.0;}
  ...
  public double score (FiniteStates X, Configuration conf){
	...
	
	{
		...
		for(k=0 ; k< nStatesX ; k++){
		  ...
		  if(Nijk>0) { sum+=Nijk*Math.log(Nijk+3.0/nStatesX-1.0);}
                    Nij+= Nijk;
	            //try{System.in.read();}catch(IOException e){};
		}
		...
		if (Nij>0) {partialSum = -Nij*Math.log((double)Nij+3.0);}
                else {partialSum=0.0;}
		sum+=partialSum;
