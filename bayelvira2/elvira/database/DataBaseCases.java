...
import elvira.learning.classification.supervised.discrete.HierarchyStructure;
...
public PotentialTable getPotentialTable(NodeList vars, Configuration condition) {
    ...
    /**
     * This method returns an array with the absolute frequencies for two variables
     * but considering only the cases compatible with a given configuration.
     * Registers with missing values are not considered.
     *
     * @param X
     ...
     * @param conf
     *            the configuration to which the database is restricted
     * @return an <code> array </code> with two indexes  with the frequencies.
     */
    
    public double[][] getFreq(FiniteStates X, FiniteStates Y, Configuration conf) {
     ...
     //   boolean missingValues;
        ...
        result = new double[X.getNumStates()][Y.getNumStates()];
        ...
        for (int cas = 0; cas < getNumberOfCases(); cas++) {
            confi = cases.get(cas);
             ...
             if (conf.isCompatibleWeak(confi)) {
                i = confi.getValue(posx);
                 j = confi.getValue(posy);
                if ( (i != -1) && (j!=-1)  ){
                    result[i][j]++;
                }
             }
    ...
    public double[][] getFreq(FiniteStates X, FiniteStates Y) {
     ...
     /**
     * This method returns an array with the absolute frequencies for a variable
      *  conditioned to a list of variables
     * Registers with missing values are not considered.
     *
     * @param X
     *            a finite states node.
    ...
    public double[][] getFreq(FiniteStates X, NodeList List) {
        
         double[][] result;
         FiniteStates Y;
           NodeList vars;
         int posx,posy,listsize;
         int[] varsizes,positions;
     ...
     //   boolean missingValues;
      cases = getCases();
        ...
        for(i=0;i<List.size();i++) {
            ...
            varsizes[i] = Y.getNumStates();
            positions[i] = vars.getId(Y);
            listsize *= varsizes[i];
        }
         
        result = new double[X.getNumStates()][listsize];
        ...
        for (int cas = 0; cas < getNumberOfCases(); cas++) {
            confi = cases.get(cas);
                ...
                for(k=0;k<listsize;k++) {
                    if (k>0) {j *= varsizes[k-1];}
                    l = confi.getValue(positions[k]);
                    if (l==-1) {
                        j= -1;
                        break;
                    }
    ...
    /**
     * This method returns an array with the absolute frequencies for one variables
     * but considering only the cases compatible with a given configuration.
     * Registers with missing values are not considered.
     *
     * @param X
    ...
    public double[] getFreq(FiniteStates X, Configuration conf) {
     ...
     //   boolean missingValues;
        ...
        result = new double[X.getNumStates()];
    ...
    public double[] getFreq(FiniteStates X) {
    ...
    /**
     * This method returns an array with the absolute frequencies for 3 variables
     * 
     * Registers with missing values are not considered.
     *
     * @param X
    ...
    public double[][][] getFreq(FiniteStates X, FiniteStates Y, FiniteStates Z) {
     ...
     //   boolean missingValues;
        ...
        result = new double[X.getNumStates()][Y.getNumStates()][Z.getNumStates()];
        
        for(i=0;i<X.getNumStates();i++) {
            for(j=0;j<Y.getNumStates();j++) {
               for(k=0;k<Z.getNumStates();k++) {
                   result[i][j][k] = 0.0;
               }
            }
        }
     ...
     dependence = new double[size][size];
       ...
       for (i=0; i<size-1; i++){
         ...
         for (j=i+1; j<size; j++)  {
           ...
           pxy = getPotentialTable(aux);
           aux.removeNode(1);
           pxy.normalize();
        /**********************************************************/
         
           dependence[i][j] =pxy.crossEntropyPotential();
           dependence[j][i] = dependence[i][j];
         }
           aux.removeNode(0);
       }
     return dependence;
...
/*      It simply computes the p-value of the chi-square test minus 0.99*/
    ...
    public DataBaseCases transform(FiniteStates classVar, HierarchyStructure tree) {
       int[] mapping;
       boolean[] members;
       int current;
       ...
       pos=0;
        ...
        while (oldcases.hasNext()){
           ...
           if (members[oldconf.getValue(pos)]){
               ...
               newconf.getValues().setElementAt( new Integer (mapping[ ((Integer) oldconf.getValues().elementAt(pos)).intValue()    ]),pos);
